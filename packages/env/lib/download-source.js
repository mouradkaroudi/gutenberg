 ...
 * on the source type. The source is downloaded to source.path and can be copied
 * to additioinal locations with the copyLocations argument.
 *
 * @param {Source}   source             The source to download.
 * @param {string[]} copyLocations      A list of paths to copy the download to.
 ...
 */
module.exports = async function downloadSource(
	source,
	copyLocations,
	options
) {
	...
	// Copy the download (source.path) to each specified location. These will
	// usually be copies for each environment. For example, we will likely copy
	// ~/.wp-env/foo/wordpress to ~/.wp-env/foo/tests-wordpress after download.
	await Promise.all(
		copyLocations.map( ( dir ) => copySource( source.path, dir ) )
	);
};
...
/**
 * Copies a source, taking care to ignore large directories
 * (.git, node_modules) and configuration files (wp-config.php).
 *
 * @param {string} fromPath Path to the source to copy.
